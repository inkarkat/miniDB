...
@test "when the owner starts another read transaction after his previous one timed out, a warning is printed" {
    ...
    let NOW+=5
    run -0 miniDB --start-read-transaction Trans1 --table "$BATS_TEST_NAME"
    assert_output 'Warning: Previous read transaction by Trans1 timed out 2 seconds ago but did not do any changes.'
}
