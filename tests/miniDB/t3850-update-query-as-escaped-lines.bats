...
@test "update with a new key and numerical column does not output anything when requesting full record as escaped lines" {
    ...
    run -0 miniDB --table "$BATS_TEST_NAME" --update 'quux' --column '1=This has been added' --columns 0\* --as-escaped-lines
    assert_output ''
}
...
@test "updated plain single-line key can be queried as escaped lines of the full record" {
    ...
    run -0 miniDB --table "$BATS_TEST_NAME" --update \* --column COUNT++ --columns 0\*  --as-escaped-lines
...
@test "updated single-line record with backslash can be queried as escaped lines of all non-key columns" {
    ...
    run -0 miniDB --table "$BATS_TEST_NAME" --update foo --column 'DESCRIPTION=A Foo has been updated' --columns \* --as-escaped-lines
...
@test "update of record with just a key can be queried as escaped lines of names and indices" {
    ...
    run -0 miniDB --table "$BATS_TEST_NAME" --update empty --column COUNT++ --columns '0 COUNT 3 DESCRIPTION' --as-escaped-lines
