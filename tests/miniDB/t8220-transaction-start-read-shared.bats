...
@test "when starting a new read transaction, a previous started read transaction that timed out prints a warning" {
    ...
    let NOW+=4
    run -0 miniDB --start-read-transaction Trans2 --table "$BATS_TEST_NAME"
    assert_output 'Warning: Previous read transaction by Trans1 timed out 1 second ago but did not do any changes.'
...
@test "when the owner starts another read transaction whose timeout is longer than the original one, this returns 1" {
    ...
    let NOW+=1
    run -1 miniDB --start-read-transaction Trans1 --table "$BATS_TEST_NAME"
    assert_output 'ERROR: Another read transaction by Trans1 is already in progress.'
}
...
@test "when starting a new read transaction whose timeout is longer then another's one and before another's read one times out, the attempt times out" {
    ...
    let NOW+=1
    run -5 miniDB --timeout 1 --start-read-transaction Trans2 --table "$BATS_TEST_NAME"
    assert_output 'Timed out while another read transaction by Trans1 is in progress.'
}
