...
@test "update with a new key and numerical column does not output anything when requesting full record as shell variables" {
    ...
    run -0 miniDB --table "$BATS_TEST_NAME" --update 'quux' --column '1=This has been added' --columns 0\* --as-shell-variables
    assert_output ''
}
...
@test "updated plain single-line key can be queried as shell variables of the full record" {
    ...
    run -0 miniDB --table "$BATS_TEST_NAME" --update \* --column COUNT++ --columns 0\*  --as-shell-variables
...
@test "updated single-line record with backslash can be queried as shell variables of all non-key columns" {
    ...
    run -0 miniDB --table "$BATS_TEST_NAME" --update foo --column 'DESCRIPTION=A Foo has been updated' --columns \* --as-shell-variables
...
@test "update of record with just a key can be queried as shell variables of names and indices" {
    ...
    run -0 miniDB --table "$BATS_TEST_NAME" --update empty --column COUNT++ --columns '0 COUNT 3 DESCRIPTION' --as-shell-variables
