...
@test "upgrading a transaction when another one has already been started and timed out causes error" {
    ...
    assert_output 'ERROR: Another read transaction by Trans2 has started already (and timed out 1 second ago).'
}
...
@test "upgrading a transaction when another one has already been started causes error" {
    ...
    run -6 miniDB --upgrade-to-write-transaction Trans1 --table "$BATS_TEST_NAME"
    assert_output 'ERROR: Another read transaction by Trans2 is already in progress.'
}
