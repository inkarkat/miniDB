...
@test "when inside a current transaction, reads and writes work" {
    ...
    let NOW+=1
    run -0 miniDB --within-transaction Trans1 --table "$BATS_TEST_NAME" --query foo
    assert_output 'foo	The Foo is here	42'
    let NOW+=1
    run -0 miniDB --within-transaction Trans1 --table "$BATS_TEST_NAME" --update "foo	A Foo has been updated	43"
}
...
@test "when inside an expired transaction a read action prints a warning" {
    ...
    assert_line -n 1 "foo	The Foo is here	42"
}
...
@test "when inside an expired shared transaction a read action prints a warning" {
    ...
    let NOW+=4
    run -0 miniDB --within-transaction Trans1 --table "$BATS_TEST_NAME" --query foo
    assert_line -n 0 "Warning: Current shared transaction timed out 2 seconds ago."
    assert_line -n 1 "foo	The Foo is here	42"
    let NOW+=1
    run -0 miniDB --within-transaction Trans2 --table "$BATS_TEST_NAME" --query foo
    assert_line -n 0 "Warning: Current shared transaction timed out 3 seconds ago."
    assert_line -n 1 "foo	The Foo is here	42"
}
...
@test "when inside an expired transaction a write action causes an error" {
    ...
    let NOW+=5
    run -6 miniDB --within-transaction Trans1 --table "$BATS_TEST_NAME" --update "foo	A Foo has been updated	43"
    assert_output 'ERROR: Current transaction timed out 2 seconds ago.'
}

@test "when inside an expired transaction without having started one causes error" {
    run -6 miniDB --within-transaction Trans1 --table "$BATS_TEST_NAME" --query foo
