...
@test "conflicting transaction actions print usage error" {
    ...
    [ $status -eq 2 ]
    [ "${lines[0]}" = 'ERROR: --transactional cannot be combined with the --start-read-transaction|--start-write-transaction|--within-transaction|--end-transaction|--abort-write-transaction set.' ]
    [ "${lines[2]%% *}" = 'Usage:' ]
}
